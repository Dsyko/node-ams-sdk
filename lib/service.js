...
AzureMediaService.prototype.getOutPutAssetByJob = function(jobId, cb) {
  var self = this;

  async.waterfall([
    function(next) {
      self.getJob(jobId, getResponseHandler(next));
    },
    function(job, next) {
      if (!job.OutPutAssets || !job.OutPutAssets.__deffered || !job.OutPutAssets.__deffered.uri) {
        return next(new Error('Invalid job'));
      }

      var main = job.OutPutAssets.__deffered.uri;
      var url = main.substring(main.indexOf('Jobs'), main.length);
      self.request.get(job.OutPutAssets.__deffered.uri, {}, getResponseHandler(next));
    }
  ], cb);
};
...
function getResponseHandler(next) {
