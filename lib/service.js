...
AzureMediaService.prototype.getOutPutAssetByJob = function(jobId, cb) {
  ...
  async.waterfall([
    ...
    function(job, next) {
      if (!job.OutputMediaAssets || !job.OutputMediaAssets.__deferred || !job.OutputMediaAssets.__deferred.uri) {
        return next(new Error('Invalid job'));
      }
      self.request.get(job.OutputMediaAssets.__deferred.uri, {}, getResponseHandler(next));
    },
    function(assets, next) {
      if (!assets.results) {
        return next(new Error('Invalid output assets'));
      }
      next(null, assets.results[0]);
    }
  ], cb);
};
